...
func Open(ctx context.Context, path string) (*Store, error) {
	...
	store.Owed = &OwedLedgerRepo{store: store}
	store.OwedTx = &OwedTransactionRepo{store: store}
...
func (s *Store) withQueries(q *db.Queries) *Store {
	...
	txStore.Owed = &OwedLedgerRepo{store: txStore}
	txStore.OwedTx = &OwedTransactionRepo{store: txStore}
...
func (s *Store) validateSchema(ctx context.Context) error {
	for _, table := range []string{"app_meta", "currencies", "currency_rates", "accounts", "balances", "history", "tags", "account_tags", "budget_categories", "budgets", "budget_allocations", "transactions", "transaction_tags", "owed_ledgers", "owed_transactions"} {
		var name string
